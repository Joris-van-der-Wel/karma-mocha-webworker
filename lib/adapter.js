...
karma.start = () => {
        ...
        function doEvaluateHook(phase) {
                if (myConfig.evaluate && myConfig.evaluate[phase]) {
                        rpc.notify('eval', myConfig.evaluate[phase]);
                }
        }

        rpc.addNotifyMethods({
                ready() {
                        doEvaluateHook('beforeMochaImport');
                        rpc.notify('importScripts', [mochaUrl]);
                        doEvaluateHook('beforeMochaSetup');
                        rpc.notify('setupMocha', myConfig.mocha);
                        doEvaluateHook('beforeScripts');
                        rpc.notify('importScripts', userScriptUrls);
                        doEvaluateHook('beforeRun');
                        rpc.notify('runMocha', karma.config.args);
                },

                mochaRunComplete() {
                        doEvaluateHook('afterRun');
                },
